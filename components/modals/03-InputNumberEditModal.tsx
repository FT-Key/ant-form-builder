...
export default function InputNumberEditModal({
  ...
  const { activePanels, setActivePanels } = useCollapsePanels(
    // errores básicos
    ["errorLabel", "errorName", "errorPlaceholder", "errorMin", "errorMax"],
    // errores avanzados
    [
      "errorStep",
      "errorPrecision",
  ...
  const { errors, validateAndSave } = useInputValidation({
    ...
    onSave,
    buildCode: () => buildInputCode(localFields, {}, "InputNumber"),
  });
  ...
  useEffect(() => {
    ...
    const matchNumberProp = (attr: string): number | undefined => {
      const brace = codeBlock.match(new RegExp(`${attr}={(\\d+\\.?\\d*)}`));
    ...
    setLocalFields((prev) => ({
      ...
      min: matchNumberProp("min"),
      max: matchNumberProp("max"),
      step: matchNumberProp("step"),
      precision: matchNumberProp("precision"),
      disabled: matchBool("disabled"),
      readOnly: matchBool("readOnly"),
      autoFocus: matchBool("autoFocus"),
      keyboard: !codeBlock.includes("keyboard={false}"),
      controls: !codeBlock.includes("controls={false}"),
      size:
  ...
  const handleSave = () => {
    ...
    if (!hasAnyErrors) {
      onSave(buildInputCode(localFields, {}, "InputNumber"));
      return true;
    }
    return false;
  };
